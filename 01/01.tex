...
\subsection{Übungsblätter}
	\begin{frame}{Übungsblätter}
		\textbf{Ausgabe:} Mittwochs im ILIAS-Forum \\ %TODO
		\pause
		\textbf{Abgabe:}
			\begin{itemize}
...
\subsection{Basics}
	...
	\begin{frame}{Mengen}
		\begin{block}{Def.: Element}
			Ein Objekt $x$, dass in einer Menge $M$ enthalten ist, heißt \textbf{Element von $M$}. Man schreibt $x \in M$ oder $x \notin M$.			
		\end{block}
...
\subsection{Operationen}
	\begin{frame}{Mengen}
		\begin{block}{Def.: Kardinalität}
			Unter \textbf{Kardinalität} einer Menge $A$ versteht man die Anzahl der Elemente der Menge. Man schreibt $\setsize{M}$ .
		\end{block}
		\pause
		\begin{exampleblock}{Beispiele}
			\begin{itemize} % TODO \pause -> \only
				\item $\setsize{SW} = \pause 8$
				\item $\setsize{\emptyset} = \pause 0$
				\item $\setsize{\{1,2,2,3\}} = \pause 3$
			\end{itemize}
		\end{exampleblock}
	\end{frame}
	...
	\begin{frame}{Mengen}
		...
		\begin{exampleblock}{Beispiele}
			\begin{itemize} % TODO \pause -> \only
				%\item $\{1,3,5\}\cup\{2,4,6\} = \{1,2,3,4,5,6\}$
				\item $\{4,5,6\}\cup\{1,2,3\}\cup\{7\}\cup\{RO\} = \pause \{1,2,3,RO,4,5,6,7\}$
				\item $\{1,2,3\}\cup\{2,3,4\} = \pause \{1,2,3,4\}$
				\item Für jede Menge $M$ gilt: $M \cup \emptyset = \pause M$
			\end{itemize}
		\end{exampleblock}
	\end{frame}
	...
	\begin{frame}{Mengen}
		...
		\begin{exampleblock}{Beispiele}
			\begin{itemize} % TODO \pause -> \only?
				\item $\{1,2,3\}\cap\{1,2\} = \pause \{1,2\}$
				\item Für jede Menge $M$ gilt: $M \cup \emptyset = \pause \emptyset$
			\end{itemize}
		\end{exampleblock}
	\end{frame}	
