...
\begin{frame}{Zweierkomplement}
	\begin{block}{Def.: $\text{Zkpl}_\ell$}
		...
		Äquivalent:
		$$\text{Zkpl}_\ell(x) = \begin{cases} \text{bin}_{\ell}(x) & \text{, falls } x \geq 0 \\ \text{bin}_{\ell}(2^{l}+x) & \text{, falls } x < 0\end{cases}$$
		wobei
		\[
...
\begin{frame}{Darstellung von Zahlen}
    \begin{exampleblock}{Aufgabe}
    	...
    	\begin{enumerate}
     		\item Gib die Dezimaldarstellung an, die $u$ und $v$ als Binärdarstellung haben. Gib ein Wort $w \in Z_2^*$ an, sodass $Num_2(u)+Num_2(v)=Num_2(w)$ gilt.
    		\item Gib den Dezimalwert der Zkpl-Interpretation (der Länge $5$) von $u, v, w$ an, also $\text{Num}_{Zkpl}(x)$ für $x\in \set{u,v,w}$.
