...
\begin{frame}{zu ÜB 4 Aufg. 1 + 2}
	\begin{itemize}
		...
		\only<2>{
			\item \textbf{Falsch:} $L_{1} = \set{ \set{a,b}^{\ast} \cdot \set{aa,bb,ab,ba} }$
		}
		\only<3-|handout:0>{
			\item \sout{$L_{1} = \set{ \set{a,b}^{\ast} \cdot \set{aa,bb,ab,ba} }$} 
			\item \textbf{Korrekt: } $L_{1} = \set{a,b}^{\ast} \cdot \set{aa,bb,ab,ba}$
		}
	\end{itemize}
\end{frame}
